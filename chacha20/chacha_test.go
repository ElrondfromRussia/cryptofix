...
func TestNoOverlap(t *testing.T) {
	for _, c := range testVectors {
		...
		err := s.XORKeyStream(output, input)
		if err != nil {
			t.Fatal(err)
		}
...
func TestStep(t *testing.T) {
	...
	for _, c := range testVectors {
		...
		for i+step < len(input) {
			err := s.XORKeyStream(output[i:i+step], input[i:i+step])
			if err != nil {
				t.Fatal(err)
			}
		...
		err := s.XORKeyStream(output[i:], input[i:])
		if err != nil {
			t.Fatal(err)
		}
		// ensure we tolerate a call with an empty input
		err = s.XORKeyStream(output[len(output):], input[len(input):])
		if err != nil {
			t.Fatal(err)
		}
...
func TestSetCounter(t *testing.T) {
	...
	err = s.XORKeyStream(dst2[64:], src[64:])
	if err != nil {
		t.Fatal(err)
	}
	...
	s = newCipher()
	err = s.XORKeyStream(dst1[:70], src[:70])
	if err != nil {
		t.Fatal(err)
	}
	s = newCipher()
	err = s.XORKeyStream([]byte{0}, []byte{0})
	if err != nil {
		t.Fatal(err)
	}
	err = s.SetCounter(1)
	...
	err = s.XORKeyStream(dst2[64:70], src[64:70])
	if err != nil {
		t.Fatal(err)
	}
	...
	err = s.SetCounter(0)
	if err == nil {
		t.Error("counter decreasing should trigger an error")
	}
}
...
func TestLastBlock(t *testing.T) {
	...
	// trigger a panic
	s, _ := NewUnauthenticatedCipher(make([]byte, KeySize), make([]byte, NonceSize))
	err := s.SetCounter(0xffffffff)
	if err != nil {
		t.Fatal(err)
	}
	blocks := make([]byte, blockSize*2)
	err = s.XORKeyStream(blocks, blocks)
	if err == nil {
		t.Error("crypting multiple blocks should trigger an error")
	}

	// setting the counter to 0xffffffff - 1 and crypting two blocks should not
	// trigger a panic
	s, _ = NewUnauthenticatedCipher(make([]byte, KeySize), make([]byte, NonceSize))
	err = s.SetCounter(0xffffffff - 1)
	if err != nil {
		t.Fatal(err)
	}
	err = s.XORKeyStream(blocks, blocks)
	if err != nil {
		t.Error("crypting the last blocks should not trigger an error")
	}
	checkLastBlock(blocks[blockSize:])
	// once all the keystream is spent, setting the counter should panic
	err = s.SetCounter(0xffffffff)
	if err == nil {
		t.Error("setting the counter after overflow should trigger an error")
	}
	// crypting a subsequent block *should* panic
	block := make([]byte, blockSize)
	err = s.XORKeyStream(block, block)
	if err == nil {
		t.Error("crypting after overflow should trigger an error")
	}
	...
	err = s.XORKeyStream(block[:7], block[:7])
	if err != nil {
		t.Error("crypting part of the last block should not trigger an error")
	}
	err = s.XORKeyStream(block[7:], block[7:])
	if err != nil {
		t.Error("crypting part of the last block should not trigger an error")
	...
	err = s.XORKeyStream(block[:1], block[:1])
	if err == nil {
		t.Error("crypting after overflow should trigger an error")
	}
}
...
func benchmarkChaCha20(b *testing.B, step, count int) {
	...
	for i := 0; i < b.N; i++ {
		...
		for i := 0; i < tot; i += step {
			err := c.XORKeyStream(dst[i:], src[i:i+step])
			if err != nil {
				b.Fatal(err)
			}
		}
