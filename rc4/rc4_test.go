...
func TestBlock(t *testing.T) {
	...
	var err error
	for i := range data1 {
		err = c1a.XORKeyStream(data1[i:i+1], data1[i:i+1])
		err = c1b.XORKeyStream(data1[i:i+1], data1[i:i+1])
	}

	c2a, _ := NewCipher(golden[0].key)
	c2b, _ := NewCipher(golden[1].key)
	data2 := make([]byte, 1<<20)
	err = c2a.XORKeyStream(data2, data2)
	err = c2b.XORKeyStream(data2, data2)

	if !bytes.Equal(data1, data2) || err != nil {
		t.Fatalf("bad block")
	}
}
