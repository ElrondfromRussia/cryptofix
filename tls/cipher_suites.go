...
import (
	...
	"github.com/ElrondfromRussia/cryptofix/des"
	"github.com/ElrondfromRussia/cryptofix/hmac"
	"github.com/ElrondfromRussia/cryptofix/rc4"
	...
	"github.com/ElrondfromRussia/cryptofix/chacha20poly1305"
)
...
type cipherSuite struct {
	...
	flags  int
	cipher func(key, iv []byte, isRead bool) (interface{}, error)
...
var cipherSuitesTLS13 = []*cipherSuiteTLS13{
	{TLS_AES_128_GCM_SHA256, 16, aeadAESGCMTLS13, cryptofix.SHA256},
	{TLS_CHACHA20_POLY1305_SHA256, 32, aeadChaCha20Poly1305, cryptofix.SHA256},
	{TLS_AES_256_GCM_SHA384, 32, aeadAESGCMTLS13, cryptofix.SHA384},
}

func cipherRC4(key, iv []byte, isRead bool) (interface{}, error) {
	cipherP, _ := rc4.NewCipher(key)
	return cipherP, nil
}

func cipher3DES(key, iv []byte, isRead bool) (interface{}, error) {
...
func cipherAES(key, iv []byte, isRead bool) (interface{}, error) {
...
func (f *prefixNonceAEAD) Seal(out, nonce, plaintext, additionalData []byte) ([]byte, error) {
...
func (f *xorNonceAEAD) Seal(out, nonce, plaintext, additionalData []byte) ([]byte, error) {
	for i, b := range nonce {
		f.nonceMask[4+i] ^= b
	}
	result, err := f.aead.Seal(out, f.nonceMask[:], plaintext, additionalData)
	if err != nil {
		return nil, err
	}
	for i, b := range nonce {
...
func aeadAESGCM(key, noncePrefix []byte) (aead, error) {
	if len(noncePrefix) != noncePrefixLength {
		return nil, errors.New("tls: internal error: wrong nonce length")
	}
	aesP, err := aes.NewCipher(key)
	if err != nil {
		return nil, err
	}
	aead, err := cipher.NewGCM(aesP)
	if err != nil {
		return nil, err
	}
