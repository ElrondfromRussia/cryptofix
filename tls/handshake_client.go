...
func (c *Conn) clientHandshake() (err error) {
	...
	helloMursh, err := hello.marshal()
	if err != nil {
		return err
	}
	if _, err := c.writeRecord(recordTypeHandshake, helloMursh); err != nil {
		return err
	}
...
func (c *Conn) loadSession(hello *clientHelloMsg) (cacheKey string,
	...
	hashSize0, err := cipherSuite.hash.Size()
	if err != nil {
		return "", nil, nil, nil
	}
	hello.pskBinders = [][]byte{make([]byte, hashSize0)}

	// Compute the PSK binders. See RFC 8446, Section 4.2.11.2.
	hashSize, err := cipherSuite.hash.Size()
	if err != nil {
		return "", nil, nil, nil
	}
	...
	transcript := hashNew
	helloMursh, err := hello.marshalWithoutBinders()
	if err != nil {
		return "", nil, nil, nil
	}
...
func (hs *clientHandshakeState) handshake() error {
	...
	helloMursh, err := hs.hello.marshal()
	if err != nil {
		return err
	}
	hs.finishedHash.Write(helloMursh)
	servHelloMursh, err := hs.serverHello.marshal()
	if err != nil {
		return err
	}
	hs.finishedHash.Write(servHelloMursh)
...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	certMsgMursh, err := certMsg.marshal()
	if err != nil {
		return err
	}
	hs.finishedHash.Write(certMsgMursh)
	...
	if ok {
		...
		resMursh, err := cs.marshal()
		if err != nil {
			return err
		}
		hs.finishedHash.Write(resMursh)
	...
	if chainToSend != nil && len(chainToSend.Certificate) > 0 {
		...
		key, ok := chainToSend.PrivateKey.(cryptofix.Signer)
		if !ok {
			_ = c.sendAlert(alertInternalError)
...
func (hs *clientHandshakeState) establishKeys() error {
	...
	if hs.suite.cipher != nil {
		clientCipher, err = hs.suite.cipher(clientKey, clientIV, false /* not for reading */)
		if err != nil {
			return err
		}
		clientHash = hs.suite.mac(c.vers, clientMAC)
		serverCipher, err = hs.suite.cipher(serverKey, serverIV, true /* for reading */)
		if err != nil {
			return err
		}
...
func (hs *clientHandshakeState) readFinished(out []byte) error {
	...
	resMursh, err := serverFinished.marshal()
	if err != nil {
		return err
	}
	hs.finishedHash.Write(resMursh)
...
func (hs *clientHandshakeState) readSessionTicket() error {
	...
	resMursh, err := sessionTicketMsg.marshal()
	if err != nil {
		return err
	}
	hs.finishedHash.Write(resMursh)
...
func (c *Conn) verifyServerCertificate(certificates [][]byte) error {
	...
	if !c.config.InsecureSkipVerify {
		...
		for _, cert := range certs[1:] {
			err := opts.Intermediates.AddCert(cert)
			if err != nil {
				return err
			}
		}
