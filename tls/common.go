...
import (
	...
	"github.com/ElrondfromRussia/cryptofix/rand"
...
var directSigning cryptofix.Hash = 0
...
func ticketKeyFromBytes(b [32]byte) (key ticketKey, err error) {
	hashed, err := sha512.Sum512(b[:])
	if err != nil {
		return key, err
	}
...
func (c *Config) serverInit(originalConfig *Config) error {
	...
	if originalConfig != nil {
		...
		keyP, err := ticketKeyFromBytes(c.SessionTicketKey)
		if err != nil {
			return err
		}
		c.sessionTicketKeys = []ticketKey{keyP}
	}
	return nil
}
...
// running in order to rotate the session ticket keys. The function will error
// if keys is empty.
func (c *Config) SetSessionTicketKeys(keys [][32]byte) error {
	...
	var err error
	for i, bytesP := range keys {
		newKeys[i], err = ticketKeyFromBytes(bytesP)
		if err != nil {
			return err
		}
	}
...
func (chi *ClientHelloInfo) SupportsCertificate(c *Certificate) error {
	...
	supportsRSAFallback := func(unsupported error) error {
		...
		if priv, ok := c.PrivateKey.(cryptofix.Decrypter); ok {
	...
	if priv, ok := c.PrivateKey.(cryptofix.Signer); ok {
...
type Certificate struct {
	...
	// an RSA PublicKey.
	PrivateKey cryptofix.PrivateKey
