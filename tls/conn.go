...
func (hc *halfConn) incSeq() error {
	...
	return errors.New("TLS: sequence number wraparound")
}
...
func (hc *halfConn) decrypt(record []byte) ([]byte, recordType, error) {
	...
	if hc.cipher != nil {
		switch c := hc.cipher.(type) {
		case cipher.Stream:
			err := c.XORKeyStream(payload, payload)
			if err != nil {
				return nil, 0, err
			}
		case aead:
		...
		case cbcMode:
			...
			err := c.CryptBlocks(payload, payload)
			if err != nil {
				return nil, 0, err
			}
...
func (hc *halfConn) encrypt(record, payload []byte, rand io.Reader) ([]byte, error) {
	...
	case cipher.Stream:
		...
		err := c.XORKeyStream(dst[:len(payload)], payload)
		if err != nil {
			return nil, err
		}
		err = c.XORKeyStream(dst[len(payload):], mac)
		if err != nil {
			return nil, err
		}
	case aead:
		...
		if hc.version == VersionTLS13 {
			...
			var err error
			record, err = c.Seal(record[:recordHeaderLen],
			...
			var err error
			record, err = c.Seal(record, nonce, payload, hc.additionalData[:])
			if err != nil {
				return nil, err
			}
		}
...
func (c *Conn) maxPayloadSizeForWrite(typ recordType) (int, error) {
	...
	toSubstract, err := c.out.explicitNonceLen()
	if err != nil {
		return maxPlaintext, err
	}
	payloadBytes := tcpMSSEstimate - recordHeaderLen - toSubstract
	if c.out.cipher != nil {
		...
		case cbcMode:
			...
			payloadBytes -= c.out.mac.Size()
		default:
			return maxPlaintext, errors.New("unknown cipher type")
		}
	}
	if c.vers == VersionTLS13 {
	...
	c.packetsSent++
	if pkt > 1000 {
		return maxPlaintext, nil // avoid overflow in multiply below
	}
...
func (c *Conn) handleKeyUpdate(keyUpdate *keyUpdateMsg) error {
	...
	if keyUpdate.updateRequested {
		...
		if err != nil {
			...
			_ = c.out.setErrorLocked(err)
			return nil
		}
		_, err = c.writeRecordLocked(recordTypeHandshake, bts)
		if err != nil {
			// Surface the error at the next write.
			_ = c.out.setErrorLocked(err)
			return nil
