...
import (
	...
	_ "github.com/ElrondfromRussia/cryptofix/md5"
...
func TestPSSGolden(t *testing.T) {
	...
	h, err := hash.New()
	if err != nil {
		t.Errorf("#error while TestPSSGolden: %s", err)
		return
	}
...
func TestPSSOpenSSL(t *testing.T) {
	...
	h, err := hash.New()
	if err != nil {
		t.Errorf("#error while TestPSSOpenSSL: %s", err)
		return
	}
...
func TestPSSNilOpts(t *testing.T) {
	...
	h, err := hash.New()
	if err != nil {
		t.Errorf("#error while TestPSSNilOpts: %s", err)
		return
	}
	...
	_, _ = SignPSS(rand.Reader, rsaPrivateKey, hash, hashed, nil)
}
...
func TestPSSSigning(t *testing.T) {
	...
	h, err := hash.New()
	if err != nil {
		t.Errorf("#error while signing: %s", err)
		return
	}
	h.Write([]byte("testing"))
	hashed := h.Sum(nil)
