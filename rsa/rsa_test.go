...
func TestImpossibleKeyGeneration(t *testing.T) {
	...
	// an infinite loop.
	for i := 0; i < 32; i++ {
		_, _ = GenerateKey(rand.Reader, i)
		_, _ = GenerateMultiPrimeKey(rand.Reader, 3, i)
		_, _ = GenerateMultiPrimeKey(rand.Reader, 4, i)
		_, _ = GenerateMultiPrimeKey(rand.Reader, 5, i)
	}
}
...
func BenchmarkRSA2048Sign(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _ = SignPKCS1v15(rand.Reader, test2048Key, cryptofix.SHA256, hashed[:])
	}
}
...
func TestEncryptOAEP(t *testing.T) {
	...
	for i, test := range testEncryptOAEPData {
		...
		for j, message := range test.msgs {
			...
			out, err := EncryptOAEP(sha1P, randomSource, &public, message.in, nil)
			if err != nil {
...
func TestDecryptOAEP(t *testing.T) {
	...
	for i, test := range testEncryptOAEPData {
		...
		for j, message := range test.msgs {
			out, err := DecryptOAEP(sha1P, nil, private, message.out, nil)
			if err != nil {
				t.Errorf("#%d,%d error: %s", i, j, err)
			...
			out, err = DecryptOAEP(sha1P, random, private, message.out, nil)
			if err != nil {
