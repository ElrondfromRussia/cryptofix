...
func TestVectors(t *testing.T) {
	for i, test := range chacha20Poly1305Tests {
		...
		ct, err := aead.Seal(nil, nonce, plaintext, ad)
		if err != nil {
			t.Fatal(err)
		}
...
func benchamarkChaCha20Poly1305Seal(b *testing.B, buf []byte, nonceSize int) {
	...
	var err error
	for i := 0; i < b.N; i++ {
		out, err = aead.Seal(out[:0], nonce[:], buf[:], ad[:])
		if err != nil {
			panic(err)
		}
	}
...
func benchamarkChaCha20Poly1305Open(b *testing.B, buf []byte, nonceSize int) {
	...
	ct, err := aead.Seal(ct[:0], nonce[:], buf[:], ad[:])
	if err != nil {
		panic(err)
	}
...
func ExampleNewX() {
	...
	var encryptedMsg []byte
	{
		...
		encryptedMsg, err = aead.Seal(nonce, nonce, msg, nil)
		if err != nil {
			panic(err)
		}
	}
