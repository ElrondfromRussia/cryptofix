...
func TestSignVerify(t *testing.T) {
	...
	sig, _ := Sign(private, message)
	if ok, _ := Verify(public, message, sig); !ok {
		t.Errorf("valid signature rejected")
	}

	wrongMessage := []byte("wrong message")
	if ok, _ := Verify(public, wrongMessage, sig); ok {
...
func TestCryptoSigner(t *testing.T) {
	...
	signer := cryptofix.Signer(private)
	...
	var noHash cryptofix.Hash
	signature, err := signer.Sign(zero, message, noHash)
	if err != nil {
		t.Fatalf("error from Sign(): %s", err)
	}

	if ok, _ := Verify(public, message, signature); !ok {
...
func TestGolden(t *testing.T) {
	...
	for scanner.Scan() {
		...
		if ok, _ := Verify(pubKey, msg, sig2); !ok {
			t.Errorf("signature failed to verify on line %d", lineNo)
		}

		priv2, _ := NewKeyFromSeed(priv[:32])
...
func TestMalleability(t *testing.T) {
	...
	if ok, _ := Verify(publicKey, msg, sig); ok {
