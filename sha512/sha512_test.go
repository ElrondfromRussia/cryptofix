...
func TestGolden(t *testing.T) {
	tests := []struct {
		...
		{
			"SHA512/224",
			func(in []byte) []byte { a, _ := Sum512_224(in); return a[:] },
			New512_224(),
			golden224,
		},
		{
			"SHA512/256",
			func(in []byte) []byte { a, _ := Sum512_256(in); return a[:] },
			New512_256(),
			golden256,
		},
		{
			"SHA384",
			func(in []byte) []byte { a, _ := Sum384(in); return a[:] },
			New384(),
			golden384,
		},
		{
			"SHA512",
			func(in []byte) []byte { a, _ := Sum512(in); return a[:] },
			New(),
			golden512,
		},
	}
