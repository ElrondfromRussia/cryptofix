...
func newGCMWithNonceAndTagSize(cipher Block, nonceSize, tagSize int) (AEAD, error) {
	...
	if cipher, ok := cipher.(gcmAble); ok {
		ncipher, err := cipher.NewGCM(nonceSize, tagSize)
		if err != nil {
			return nil, errors.New("cipher: incorrect NewGCM")
		}
		return ncipher, nil
	}
...
func (g *gcm) Seal(dst, nonce, plaintext, data []byte) ([]byte, error) {
	if len(nonce) != g.nonceSize {
		return nil, errors.New("crypto/cipher-incorrect nonce length given to GCM")
	}
	if uint64(len(plaintext)) > ((1<<32)-2)*uint64(g.cipher.BlockSize()) {
		return nil, errors.New("crypto/cipher-message too large for GCM")
	}

	ret, out := sliceForAppend(dst, len(plaintext)+g.tagSize)
	if subtleoverlap.InexactOverlap(out, plaintext) {
		return nil, errors.New("crypto/cipher-invalid buffer overlap")
	}
